...
class DisasmFunc:
    __slots__ = ('name', 'nargs', 'nvars', 'codelen', 'natoms', 'nsrc',
                 'nconst', 'nobj', 'nreg', 'ntry', 'nblk', 'sbits',
                 'argvs', 'var_slot_names', 'atoms', 'consts', 'ops',
                 'children', 'source_path', 'source_text', 'is_cocos')
    ...
    def to_text(self):
        ...
        lines.append(f'.meta nargs={self.nargs} nvars={self.nvars} codelen={self.codelen} '
        ...
        for line in text.splitlines():
            ...
            if stripped.startswith('.func '):
                ...
                for part in stripped[6:].split():
                    k, v = part.split('=', 1)
                    v = int(v)
                    if k == 'nargs': func.nargs = v
                    elif k == 'nvars': func.nvars = v
                    elif k == 'codelen': func.codelen = v
                    elif k == 'natoms': func.natoms = v
                    elif k == 'nsrc': func.nsrc = v
                    elif k == 'nconst': func.nconst = v
                    elif k == 'nobj': func.nobj = v
                    elif k == 'nreg': func.nreg = v
                    elif k == 'ntry': func.ntry = v
                    elif k == 'nblk': func.nblk = v
                    elif k == 'sbits': func.sbits = v
                    elif k == 'cocos': func.is_cocos = bool(v)
                ...
                if ct == 'int':
                    ...
                    cv = _unesc(stripped[stripped.index('"') + 1:stripped.rindex('"')])
                elif ct == 'bool':
                    cv = parts[3] == 'True'
                elif ct == 'null':
                    cv = None
