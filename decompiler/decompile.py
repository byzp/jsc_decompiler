...
class DecompileEngine:
    def __init__(self, func, parent_func=None, dump_bytecode=False):
        ...
        self._in_catch = False  # inside catch block (between exception and finally/leaveblock)
        self._catch_closed = False  # catch block's } already emitted by leaveblock
        self._switch_labels = {}  # target_offset → 'case val:' / 'default:'
        self._switch_stack = []  # stack of (in_switch, switch_labels, switch_default_target, switch_ifs_start, block_depth_at_switch) for nested switches
        ...
        self._iter_loops = {}  # iternext_offset → {iter_offset, obj_value, flags, loop_var, loop_end}
        self._iter_suppress_setlocal = None  # localno to suppress (from iternext+setlocal pattern)
        ...
        for i in range(n):
            ...
            if iternext_off is not None:
                ...
                if moreiter_off is not None and loop_end_off is not None:
                    for k in range(n):
                        if ops[k]['off'] >= moreiter_off and ops[k]['off'] <= loop_end_off:
                            if ops[k]['nm'] == 'pop':
    ...
    _JS_RESERVED = frozenset({
        ...
        'static', 'await', 'null', 'true', 'false', 'set', 'get',
    })
    ...
    def _is_ident(s):
        ...
        if s[0] not in '_$abcdefghijklmnopqrstuvwxyzABCDEFGHIJKLMNOPQRSTUVWXYZ':
            return False
        return all(c in '_$abcdefghijklmnopqrstuvwxyzABCDEFGHIJKLMNOPQRSTUVWXYZ0123456789' for c in s)
    ...
    def _dispatch(self, op):
        ...
        # property access
            ...
            self._push(tp='script',
                       script=f'{ov}{aname_expr}={val_str}')
        elif nm == 'delprop':
            obj = self._pop()
            ov = obj.get_value()
            if _is_numeric(ov):
                ov = '(' + ov + ')'
            self._push(tp='script',
                       script=f'delete {ov}{self._prop(p.get("idx",0))}')
            ...
            if _is_numeric(ov):
                ov = '(' + ov + ')'
            self._push(tp='script',
                       script=f'{ov}[{idx.get_value()}]={val.get_value()}')
        elif nm == 'delelem':
            idx = self._pop(); obj = self._pop()
            ov = obj.get_value()
            if _is_numeric(ov):
                ov = '(' + ov + ')'
            self._push(tp='script',
                       script=f'delete {ov}[{idx.get_value()}]')
        elif nm == 'length':
            obj = self._pop()
            ov = obj.get_value()
            if _is_numeric(ov):
        ...
        # variable declarations
            ...
            is_gen = idx < len(self.d.children) and (
                bool(self.d.children[idx].sbits & (1 << 8)) or
                any(op['nm'] == 'yield' for op in self.d.children[idx].ops))
        ...
        # args / locals
            ...
            if self._iter_suppress_setlocal is not None and self._iter_suppress_setlocal == ln:
        ...
        # Postfix: Xinc, Xdec (e.g. arginc, localdec, propinc, eleminc, aliasedvarinc, aliasedvardec)
            ...
            elif nm in ('incname', 'decname', 'nameinc', 'namedec',
                        'incgname', 'decgname', 'gnameinc', 'gnamedec'):
                name = self._push_name(p.get('idx', 0))
                if is_prefix:
                    self._push(tp='script', script=f'{op}{name}')
        ...
        # aliased vars
            ...
            if not var_name:
                var_name = f'_av{p.get("slot", 0)}'
            self._push(tp='script', name=var_name, script=f'{var_name}={val.get_value()}')
        ...
        # misc
            ...
            # Check if the next opcode is setlocal/setaliasedvar/setname — that's our loop variable
            loop_var = '_it'
            ops = self.d.ops
            ...
            if o in self._iter_loops:
                ...
                self._iter_loops[o]['obj_value'] = getattr(self, '_current_iter_obj', '')
