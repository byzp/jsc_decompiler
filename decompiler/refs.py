...
_RE_VAR_DEF_EXPR = re.compile(r'^(\w+)\s*=\s*\((\w+)\s*\+\s*\"([^\"]+)\"\s*\)', re.MULTILINE)
...
def _resolve_path(current_rel, filename, files):
    ...
    if len(candidates) == 1:
        ...
        return min(candidates, key=lambda c: _path_distance(cur_dir, os.path.dirname(c)))
    return None


def _resolve_simple(current_rel, filename, files):
    """Find which file 'filename' maps to in the directory tree."""
    candidates = [rel for rel in files if rel.endswith('/' + filename) or rel == filename]
    if len(candidates) == 1:
        return candidates[0]
    elif candidates:
        cur_dir = os.path.dirname(current_rel)
        best = min(candidates, key=lambda c: _path_distance(cur_dir, os.path.dirname(c)))
        return best
    return None
