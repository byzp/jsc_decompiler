...
def _parse_header_and_atoms(data):
    ...
    o = 4
    nargs = struct.unpack_from('<H', data, o)[0]; o += 2
    nbl = struct.unpack_from('<H', data, o)[0];   o += 2
    nvars = u32le(data, o);   o += 4
    clen = u32le(data, o);    o += 4
    prolog = u32le(data, o);  o += 4
    jsver = u32le(data, o);   o += 4
    natoms = u32le(data, o);  o += 4
    nsrc = u32le(data, o);    o += 4
    nconst = u32le(data, o);  o += 4
    nobj = u32le(data, o);    o += 4
    nreg = u32le(data, o);    o += 4
    ntry = u32le(data, o);    o += 4
    nblk = u32le(data, o);    o += 4
    nts = u32le(data, o);     o += 4
    flen = u32le(data, o);    o += 4
    sbits = u32le(data, o);   o += 4
    ...
    if nul < 0 or nul - o > 512:
        nul = o
    func.source_path = data[o:nul].decode('latin-1', errors='replace') if nul > o else ''
    sub_start = nul + 1
...
def _parse_consts(data, func, start_off):
    ...
    for _ in range(func.nconst):
        if o + 4 > len(data):
            break
        type_tag = struct.unpack_from('<I', data, o)[0]; o += 4
        if type_tag == 0:
            v = struct.unpack_from('<I', data, o)[0] if o + 4 <= len(data) else 0; o += 4
            func.consts.append(('int', v))
        elif type_tag == 1:
            v = struct.unpack('<d', data[o:o + 8])[0] if o + 8 <= len(data) else 0.0; o += 8
